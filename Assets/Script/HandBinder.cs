...
public class HandBinder : MonoBehaviour
{
    ...
    public void UpdateHandPoints(Vector3[] landmarks, bool isLeft)
    {
        ...
        if (isLeft)
        {
            leftHandPoints = worldPoints;
        }
        else
        {
            rightHandPoints = worldPoints;
        }
    }

    public Vector3[] ConvertHandLandmarksToWorld(Vector3[] landmarks, Camera cam, float zScale = 0.3f)
    {
        Vector3[] result = new Vector3[landmarks.Length];

        for (int i = 0; i < landmarks.Length; i++)
        {
            // 注意：z 是相对深度，没有单位，所以需要乘一个缩放因子
            float x = landmarks[i].x * cam.pixelWidth;
            float y = landmarks[i].y * cam.pixelHeight; // 注意 y 方向是反的
            float z = landmarks[i].z * -zScale;
