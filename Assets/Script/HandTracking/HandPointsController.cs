...
public class HandPointsController : MonoBehaviour
{
    // 将21个小球拖拽到这个数组中
    public GameObject[] points = new GameObject[21];
    ...
    public void UpdatePoints(Vector3[] landmarks)
    {
        ...
        for (int i = 0; i < 21; i++)
        {
            ...
            {
                // 调整坐标比例和位置
                points[i].transform.localPosition = landmarks[i] * 25f; // 放大25倍
            }
        }
        DrawConnections();
    }
    ...
    {
        if (connectionLines == null || points == null) return;
        ...
        foreach (var (start, end) in _connections)
        {
            ...
            {
                linePoints.Add(points[start].transform.localPosition);
                linePoints.Add(points[end].transform.localPosition);
            }
        }
