...
public class PoseBinder : MonoBehaviour
{
    ...
    private void BulidTree()
    {
        ...
        LHip = new AvatarTree(lHip, 24, lHip.rotation);
        LKnee = LHip.child = new AvatarTree(lKnee, 26, lKnee.rotation, LHip);
        LFoot = LKnee.child = new AvatarTree(lFoot, 30, lFoot.rotation, LKnee);
        RHip = new AvatarTree(rHip, 23, rHip.rotation);
        RKnee = RHip.child = new AvatarTree(rKnee, 25, rHip.rotation, RHip);
        RFoot = RKnee.child = new AvatarTree(rFoot, 29, rFoot.rotation, RKnee);
        LSld = new AvatarTree(lSld, -7, lSld.rotation);
        LArm = LSld.child = new AvatarTree(lArm, 12, lArm.rotation, LSld);
        LEblow = LArm.child = new AvatarTree(lEblow, 14, lEblow.rotation, LArm);
        LWrist = LEblow.child = new AvatarTree(lWrist, 16, lWrist.rotation, LEblow);
        RSld = new AvatarTree(rSld, -6, rSld.rotation);
        RArm = RSld.child = new AvatarTree(rArm, 11, rArm.rotation, RSld);
        REblow = RArm.child = new AvatarTree(rEblow, 13, rEblow.rotation, RArm);
        RWrist = REblow.child = new AvatarTree(rWrist, 15, rWrist.rotation, REblow);
    }//匹配关节父子节点
